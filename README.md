...
**`.WithStandardInput(string input)`**: Provides string data to the process's standard input.
* **`.WithStandardInput(Stream inputStream)`**: Provides data from a `Stream` to the process's standard input. `ShellCrafter` does not dispose the provided stream. *(New)*
